#!/bin/bash
whiptail --title "欢迎使用Deepin-Wine-Ubuntu" --msgbox "此脚本仅供星环内部使用，未经允许请勿散布" 10 60  
rollback(){
rm -rf ~/.winetemp
}
function main_menu(){
...
if [ $exitstatus = 0 ]; then
    ...
    whiptail --title "deepin-wine提示" --msgbox "感谢使用！" 10 50
    exit 1
fi
}
function install_wine(){
	...
	#判断安装结果
		if [ `dpkg -l | grep deepin.wine |wc -l` -ge 8 ];then
			...
			whiptail --title "deepin-wine" --msgbox "安装失败，请进入.winetemp目录尝试手动执行install！" 10 60
		fi
	main_menu;
}
...
if [ $exitstatus = 0 ]; then
    ...
    1)
	    if(whiptail --title "安装确认" --yesno "请确保Deepin-Wine已经安装!!! 确认开始安装企业微信2.4.16吗?" 10 50)then
				{
					...
					echo 5
					#判断包是否准备就绪
					if [ ! -f "download/deepin.com.weixin.work_2.4.16.1347deepin1_i386.deb" ];then
					wget -q http://mirrors.aliyun.com/deepin/pool/non-free/d/deepin.com.weixin.work/deepin.com.weixin.work_2.4.16.1347deepin1_i386.deb -P download/ >/dev/null 
					fi
					sleep 1
					echo 55
					sudo dpkg -i download/deepin.com.weixin.work_2.4.16.1347deepin1_i386.deb >/dev/null
					sleep 10
				...
				whiptail --title "deepin-wine" --msgbox "安装失败，请进入download目录尝试手动安装！" 10 60
				fi
				install_menu;
			else
				main_menu;
			...
			if(whiptail --title "安装确认" --yesno "请确保Deepin-Wine已经安装!!! 确认开始安装微信2.6.2吗?" 10 50)then
				{
					sleep 1
					echo 5
					if [ ! -f "download/deepin.com.wechat_2.6.2.31deepin0_i386.deb" ];then
					wget -q http://mirrors.aliyun.com/deepin/pool/non-free/d/deepin.com.wechat/deepin.com.wechat_2.6.2.31deepin0_i386.deb -P download/ >/dev/null 
					fi
					sleep 1 
					echo 55
					sudo dpkg -i download/deepin.com.weixin.work_2.4.16.1347deepin1_i386.deb >/dev/null 
		...
		x)
			whiptail --title "警告" --msgbox "Beta功能不能保证所有程序完美运行 \ 运行时有几率导致终端未响应 \ 杀死install进程或强制重启终端即可 \ 如需卸载请手动执行[rm -rf ~/.winebuild]" 10 60
			EXE=$(whiptail --title "安装确认" --inputbox "请将exe程序放置在install目录下 输入程序全名,确认开始运行" 10 50 example.exe 3>&1 1>&2 2>&3)
				exitstatus=$?
				if [ $exitstatus = 0 ]; then
...
function update_menu(){
			...
			DEB=$(whiptail --title "安装确认" --inputbox "请将deb更新包放置在update目录下 输入更新包全名,确认开始更新" 10 50 example.deb 3>&1 1>&2 2>&3)
				exitstatus=$?
				if [ $exitstatus = 0 ]; then
	    ...
	    if(whiptail --title "卸载确认" --yesno " 确认开始卸载企业微信吗?" 10 50)then
				{
					sleep 1
					echo 5
					sudo apt remove deepin.com.weixin.work >/dev/null					
					sleep 1
					echo 55
					rm -rf ~/.deepinwine/Deepin-WXWork >/dev/null
					sleep 10
				} | whiptail --gauge "正在卸载企业微信，请稍候........." 6 60 0 &&
				if [ `dpkg -l | grep deepin.com.weixin.work |wc -l` -ge 1 ];then
	    ...
	    if(whiptail --title "卸载确认" --yesno " 确认开始卸载微信吗?" 10 50)then
				{
					sleep 1
					echo 5
					sudo apt remove deepin.com.wechat	>/dev/null	
					sleep 1
					echo 55
					rm -rf ~/.deepinwine/Deepin-WeChat >/dev/null	
					sleep 10
				} | whiptail --gauge "正在卸载微信，请稍候........." 6 60 0 &&
				if [ `dpkg -l | grep deepin.com.wechat |wc -l` -ge 1 ];then
				whiptail --title "deepin-wine" --msgbox "卸载失败，请尝试手动卸载！" 10 60
				else
				whiptail --title "deepin-wine" --msgbox "卸载完成！" 10 60
				fi
	    ...
	    if(whiptail --title "卸载确认" --yesno " 确认开始卸载deepin-wine吗?" 10 50)then
				{
					sleep 1
					echo 5
					sudo apt remove *deepin*wine* udis86 >/dev/null			
					sleep 1
					echo 55
				...
				if [ `dpkg -l | grep deepin.wine |wc -l` -ge 1 ];then
				whiptail --title "deepin-wine" --msgbox "卸载失败，请尝试手动卸载！" 10 60
				else
				whiptail --title "deepin-wine" --msgbox "卸载完成！" 10 60
				fi
